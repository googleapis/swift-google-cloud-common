...
Add `swift-google-cloud-common` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-common.git --from 0.0.0-preview
```

Then add `GoogleCloudCommon` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudCommon <target-name> --package swift-google-cloud-common
```
